# ==============================================================================
# CONFIGURACIÓN CENTRAL DEL REPORTE OTROS NEGOCIOS
# Este es el ÚNICO archivo que necesitas editar cada trimestre:
#   1. Actualiza las rutas de FILE_ACTUAL / FILE_ANTERIOR
#
# Plantilla oficial: 'IPN_Colaboradores_Otros Negocios_automatizada.pptx'.
# Todo el reporte (slides 4, 6, 7, 8 y 9) se genera sobre esa plantilla:
# shapes SL (slide 4), shapes ON (slide 6; OJO: la gráfica de barras se
# llama 'Grafico_bar_OP' en la plantilla, tal como en el script original)
# y las tablas Tabla_PRO/PAS/DET_MENS_ON (slides 7-9).
#
# NOTA: el bloque de Grupo Salinas (slide 4) filtra TIPO == 'Normal';
# el bloque de Otros Negocios (slide 6) NO filtra TIPO, fiel a los originales.
# ==============================================================================
...
TEMPLATE_PPTX = r'E:\Users\954108\Desktop\Satisfaccion  y Asuntos Publicos\Automatizaciones_PPT\IPN Colaboradores\IPN_Colaboradores_Otros Negocios_automatizada.pptx'
OUTPUT_PPTX = 'Reporte_Otros_Negocios.pptx'
...
BLOQUES_NPS = [
    ...
    # Slide 6: Otros Negocios por Nivel 3 (SIN filtro de TIPO, fiel al original;
    # la gráfica de barras se llama 'Grafico_bar_ON' en la plantilla)
    {'slide_index': 5, 'nivel_col': 'Nivel 3', 'resp_col': 'Resonsable_N3',
     'filtros': [('Agrupador', 'OTROS NEGOCIOS')],
     'etiqueta_total': 'Otros Negocios - RBS',
     'grafico_barras': 'Grafico_bar_ON', 'grafico_dona': 'Grafico _don_ON',
     'tabla_dif': 'Tabla_DIF_ON', 'tabla_anterior': 'Tabla_ANTERIO_ON', 'sufijo_dona': 'ON'},
]

# ------------------------------------------------------------------------------
# MENCIONES TOP 10 CON RUBRO (slides 7, 8 y 9)
# Una tabla por slide: motivo (color según lista), menciones (color) y
# rubro (gris). Filtro fijo del reporte: Agrupador == OTROS NEGOCIOS.
# ------------------------------------------------------------------------------
FILTRO_MENCIONES = ('Agrupador', 'OTROS NEGOCIOS')
TOP_N_MENCIONES = 10

BLOQUES_MENCIONES_RUBRO = [
    {'slide_index': 6, 'categoria': 'Promotor',  'tabla': 'Tabla_PRO_MENS_ON'},
    {'slide_index': 7, 'categoria': 'Pasivo',    'tabla': 'Tabla_PAS_MENS_ON'},
    {'slide_index': 8, 'categoria': 'Detractor', 'tabla': 'Tabla_DET_MENS_ON'},
]

# (Lista usada por el matcheo de colores; compartida con el proyecto principal)
GRUPOS_ORDENADOS = ['OTROS NEGOCIOS']
