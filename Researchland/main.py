# ==============================================================================
# ORQUESTADOR DEL REPORTE RESEARCHLAND
...
def procesar_historico_rl(prs, df_actual, errores):
    """Slide 6: actualiza el Excel histórico de Research Land e inyecta la
    gráfica de líneas con las 4 series (Promotores/IPN/Pasivos/Detractores)."""
    print("\n================ HISTÓRICO RESEARCH LAND (slide 6) ================")
    nombre = "Histórico Research Land"
    try:
        cfg = config.HISTORICO_RL
        metricas = calculos.calcular_metricas_historico_rl(df_actual, cfg['filtro'])

        print("Actualizando archivo histórico Excel...")
        df_hist = pd.read_excel(config.FILE_HISTORICO_RL)
        df_hist = calculos.actualizar_base_historica(df_hist, metricas, config.NUEVO_PERIODO)
        df_hist.to_excel(config.FILE_HISTORICO_RL, index=False)

        slide = prs.slides[cfg['slide_index']]
        inyectores.inyectar_historico(slide, cfg['grafico'], df_hist)
    except Exception:
        errores.append(nombre)
        print(f"❌ Error en {nombre}:\n{traceback.format_exc()}")
...
def ejecutar_reporte(file_actual=None, file_anterior=None, output_pptx=None,
    ...
    procesar_historico_rl(prs, df_actual, errores)
