...
# ------------------------------------------------------------------------------
# HISTÓRICO UPAX (slide 8): métricas del periodo + Excel histórico horizontal
# ------------------------------------------------------------------------------
import re as _re
import openpyxl as _openpyxl


def formatear_etiqueta_ipn(periodo_str):
    """Convierte cualquier formato de periodo al esquema visual de 2 renglones:
    '3C 2020' -> '3C\\n2020' | '2Q´26' -> '2Q\\n2026' | '2C 2026' -> '2C\\n2026'"""
    p = str(periodo_str).strip()
    if '\n' in p:
        return p

    m = _re.match(r'^(\d+[A-Za-z]+|[A-Za-z]+\d+)\s*[´\'_\-\s]?\s*(\d{2,4})$', p)
    if m:
        prefijo, anio = m.group(1), m.group(2)
        if len(anio) == 2:
            anio = f"20{anio}"
        return f"{prefijo}\n{anio}"
    ...
    if ' ' in p:
        partes = p.split()
        anio = f"20{partes[-1]}" if len(partes[-1]) == 2 else partes[-1]
        return f"{partes[0]}\n{anio}"

    return p


def calcular_metricas_periodo_upax(df_raw, filtro, col_exclusion='Nivel 4'):
    """Distribución NPS del periodo para el segmento del histórico UPAX.
    Devuelve (total_casos, conteo_nps, p_det, p_pas, p_prom, ipn)."""
    df = df_raw.copy()
    df = df[df[col_exclusion] != 'CASOS NO IDENTIFICADOS'].copy()
    df['calificacion_nps'] = df['Calificacion'].apply(clasificar_nps)
    ...
    total_casos = len(df)
    conteo_nps = df['calificacion_nps'].value_counts()

    p_prom = conteo_nps.get('Promotor', 0) / total_casos if total_casos > 0 else 0.0
    p_pas = conteo_nps.get('Pasivo', 0) / total_casos if total_casos > 0 else 0.0
    p_det = conteo_nps.get('Detractor', 0) / total_casos if total_casos > 0 else 0.0
    ipn = p_prom - p_det

    return total_casos, conteo_nps, p_det, p_pas, p_prom, ipn


def actualizar_historico_dcho(path_excel, nuevo_periodo, p_det, p_pas, p_pro, ipn_val):
    """Actualiza o agrega la columna del periodo en las 4 tablas horizontales
    del Excel histórico (Detractores filas 1-2, Pasivos 5-6, Promotores 9-10,
    IPN 13-14). Devuelve (periodos, series_historicas)."""
    wb = _openpyxl.load_workbook(path_excel)
    ...
    for col in range(2, ws.max_column + 1):
        ...
        if val is not None and str(val).strip() != "":
            periodos.append(str(val).strip())

    if nuevo_periodo in periodos:
        col_idx = periodos.index(nuevo_periodo) + 2
    else:
        col_idx = len(periodos) + 2
        ws.cell(1, col_idx).value = nuevo_periodo   # Detractores
        ws.cell(5, col_idx).value = nuevo_periodo   # Pasivos
        ws.cell(9, col_idx).value = nuevo_periodo   # Promotores
        ws.cell(13, col_idx).value = nuevo_periodo  # IPN
        periodos.append(nuevo_periodo)

    ws.cell(2, col_idx).value = round(p_det, 3)     # Fila 2: Detractor
    ws.cell(6, col_idx).value = round(p_pas, 3)     # Fila 6: Pasivo
    ws.cell(10, col_idx).value = round(p_pro, 3)    # Fila 10: Promotor
    ws.cell(14, col_idx).value = round(ipn_val, 3)  # Fila 14: IPN

    total_cols = len(periodos)

    # Asegura que las columnas históricas previas de IPN tengan su valor
    for c in range(2, total_cols + 2):
        if ws.cell(14, c).value is None:
            v_pro = ws.cell(10, c).value
            v_det = ws.cell(2, c).value
            if v_pro is not None and v_det is not None:
                ws.cell(14, c).value = round(float(v_pro) - float(v_det), 3)

    wb.save(path_excel)
    print(f"Archivo Excel actualizado: '{path_excel}' en la columna {col_idx} ({nuevo_periodo}).")

    series = {
        'Detractores': [ws.cell(2, c).value for c in range(2, total_cols + 2)],
        'Pasivos': [ws.cell(6, c).value for c in range(2, total_cols + 2)],
        'Promotores': [ws.cell(10, c).value for c in range(2, total_cols + 2)],
        'IPN': [ws.cell(14, c).value for c in range(2, total_cols + 2)],
    }
