# ==============================================================================
# APP WEB DEL REPORTE IPN COLABORADORES
#
# Cómo correrla (desde la terminal de PyCharm, dentro de la carpeta del proyecto):
#     pip install streamlit
#     streamlit run app.py
#
# Se abrirá automáticamente la URL http://localhost:8501 en tu navegador.
# Ahí seleccionas dinámicamente:
#   - FILE_ACTUAL   (base del trimestre en curso)
#   - FILE_ANTERIOR (base del trimestre anterior)
#   - NUEVO_PERIODO (etiqueta del periodo, ej. 2Q´26)
#   - OUTPUT_PPTX   (nombre del archivo de salida)
# y generas el reporte con un clic, con opción de descargarlo al terminar.
# ==============================================================================
import io
import os
import tempfile
...
st.set_page_config(page_title="Reporte IPN Colaboradores", page_icon="📊", layout="centered")

st.title("📊 Generador de Reporte IPN Colaboradores")
...
with col1:
    ...
    st.markdown(f"**Etiqueta del periodo (NUEVO_PERIODO):** `{nuevo_periodo}`")
with col2:
    output_pptx = st.text_input(
        "Nombre del archivo de salida (OUTPUT_PPTX)",
        value=f"Reporte Ejecutivo IPN Colaboradores_{trimestre} {anio}.pptx",
    )
...
with st.expander("Opciones avanzadas (plantilla)"):
    ...
    st.caption(
        "Los Excel históricos se actualizan en las rutas fijas de config.py: "
        f"{config.FILE_HISTORICO} y {config.FILE_HISTORICO_AGRUPADORES}"
    )
